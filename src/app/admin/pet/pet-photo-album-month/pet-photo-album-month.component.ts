...
@Component({
  selector: 'app-pet-photo-album-month',
  templateUrl: './pet-photo-album-month.component.html',
  styleUrl: './pet-photo-album-month.component.css'
})
export class PetPhotoAlbumMonthComponent {
  ...
  getPhotosAlbum(filterVal: any) {
    ...
    formURlData.set('month_year', this.date);
    ...
    this.service.postAPI(`sub-admin/getpetMonthlydata_photos`, formURlData.toString()).subscribe({
      next: resp => {
        if(resp.success){
          this.loading = false;
