...
@Component({
  selector: 'app-pet-photo-album-date',
  templateUrl: './pet-photo-album-date.component.html',
  styleUrl: './pet-photo-album-date.component.css'
})
export class PetPhotoAlbumDateComponent {
  ...
  getPhotosAlbum(filterVal: any) {
    ...
    formURlData.set('date_month_year', this.date);
    ...
    this.service.postAPI(`sub-admin/getpetDatedata_photos`, formURlData.toString()).subscribe({
      next: resp => {
        if(resp.success){
          this.loading = false;
