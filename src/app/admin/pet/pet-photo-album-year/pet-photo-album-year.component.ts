...
@Component({
  selector: 'app-pet-photo-album-year',
  templateUrl: './pet-photo-album-year.component.html',
  styleUrl: './pet-photo-album-year.component.css'
})
export class PetPhotoAlbumYearComponent {
  ...
  goBack(){
    this.router.navigateByUrl(`/admin/main/pet/${this.userId}/${this.ownerId}`);
  ...
  getPhotosAlbum(filterVal: any) {
    ...
    formURlData.set('pet_id', this.userId);
    //formURlData.set('user_id', '123');
    this.service.postAPI(`sub-admin/getpetYealydata_photos`, formURlData.toString()).subscribe({
      next: resp => {
        if(resp.success){
          this.loading = false;
