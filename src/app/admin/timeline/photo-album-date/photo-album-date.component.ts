...
@Component({
  selector: 'app-photo-album-date',
  templateUrl: './photo-album-date.component.html',
  styleUrl: './photo-album-date.component.css'
})
export class PhotoAlbumDateComponent {
  ...
  getPhotosAlbum() {
    ...
    this.service.postAPI(`sub-admin/get-timelinealbum`, formURlData.toString()).subscribe({
      next: resp => {
        if(resp.success){
          this.loading = false;
          this.imageObject = resp.data?.map((item: any) => ({ ...item, checked: false })).reverse();
